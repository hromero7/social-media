...
const FollowerCard = (props) => {
  ...
  const handleUnfollowUser = () => {
    ...
    return (
    <div className="card mb-3 tweet-card follower-card">
        <div className="row no-gutters">
    <div className="col-md-4">
    <Link to={`/user/profile/${props.userId}`}>
      <img src={`data:image/jpeg;base64,${props.avatar}`} className="card-img tweet-img follower-card-img" alt="..."/>
    </Link>
    </div>
    <div className="col-md-8">
      <div className="card-body follower-card-body">
      <div className="delete-btn">
      {
       user.following === undefined? null :
        ...
        <h5 className="card-title follower-card-name">{props.firstName} {props.lastName}</h5>
        <h5 className="card-title follower-card-username">@{props.username}</h5>
        <p className="card-text follower-card-bio">{props.bio}</p>
      </div>
    </div>
  </div>
</div>
